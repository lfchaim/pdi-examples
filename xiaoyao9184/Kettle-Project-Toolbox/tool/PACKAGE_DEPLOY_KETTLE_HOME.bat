...
::create command run
set c=%pdi_path%\kitchen -file:%current_script_dir%\Deploy\PackageZipDeploy4KettleHome.kjb "-param:rNameRegex=%rName%" "-param:fExcludeRegex=.*\.backup$|.*\.log$|.*\.git\\.*|.*db\.cache.*|.*data-integration.*" "-param:fIncludeRegex=.*" "-param:isOpenShell=%isOpenShell%"
